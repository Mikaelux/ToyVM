...
static void log_to_csv(int iteration, FuzzStats* stats, float reward) {
    ...
    if (f) {
        fprintf(f, "%d,%d,%d,%d,%d,%d,%.6f,%u,%.6f,%d,%d,%d\n",
                iteration,
                stats->crashes,
                stats->hangs,
                stats->asm_errors,
...
static void update_corpus(int idx, uint32_t new_vm_cov, int iteration) {
    ...
    for (int k = 0; k < corps_count; k++) {
        corpus[k].last_new_cov = (uint32_t)fmax(1.0, corpus[k].last_new_cov * 0.9);
      }
    }
}
...
// ================= REWARD COMPUTATION =================
static float compute_reward(Errors result, uint32_t vm_cov, int prog_size, bool shallow_error) {
    float reward = 0.0f;
    bool found_cov = vm_cov > 0;
    ...
    if (found_cov) {
        ...
        if (is_vm_error(result) && result != ERR_MAX_INSTRUCTIONS) {
            float depth_bonus = 0.1f * fminf(1.0f, (float)shared_cov->step_count / 500.0f);
            reward += depth_bonus;
        }
    }
    ...
    if (result == ERR_TIMEOUT) reward -= 2.0f;
    if (shallow_error) reward -= 0.5f;

    reward += 0.0001f * fminf(200.0f, (float)prog_size);
    return reward;
}
// ================= STATISTICS PRINTING =================
...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < max_iterations; i++) {
        ...
        if ((i + 1) % 1000 == 0) {
            reload_corpus();
            printf("\e[34mReloaded corpus: %d files\e[0m\n", corps_count);
        }
       ...
       Buffer* clean = buf_new((size_t)f_cor_size);
        ...
        state_update_run_stats(current_state, cov_result.vm_new, 
        ...
        reward = compute_reward(result, cov_result.vm_new, 
