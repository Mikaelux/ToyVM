...
void init_csv_log(){
  ...
  if(f){
    fprintf(f, "interation,crashes,hangs,asm_errors,vm_errors,successful,vm_cov,asm_cov\n");
    fclose(f);
  }
}

void log_to_csv(int iteration, FuzzStats*stats){
  FILE*f = fopen(MUT_CSV_LOG_FILE, "a");
  if(f){
    fprintf(f, "%d,%d,%d,%d,%d,%d,%u,%u\n",
      iteration,
      stats->crashes,
      stats->hangs,
      stats->asm_errors,
...
// save good cases in crash, hang and coverage 
...
int main(int argc, char** argv) {
    ...
    // Seed and corpus initializing, I should look into putting more weight the more recent a corp is 
